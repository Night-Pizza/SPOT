...
export default function ActiveSessionPage() {
    ...
    // Получаем студентов для текущей сессии
    ...
    const columns = useMemo<ColumnsType<Attendee>>(
        () => [
            ...
            },
            {
                ...
                key: 'email',
                render: (email: string) => (
                    <span className="student-email-cell">{email}</span>
                ),
            },
            {
                title: t('time'),
                ...
                key: 'time',
                render: (time: string) => (
                    <Space size={4} className="student-time-cell">
            ...
            <Popconfirm
                title={t('removeAttendee') || 'Remove this attendee?'}
                okText={t('remove')}
                cancelText={t('cancel')}
                onConfirm={() => {
                    if (activeSession) {
